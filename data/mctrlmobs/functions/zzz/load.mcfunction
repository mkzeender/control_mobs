scoreboard objectives add pressedbutton minecraft.used:minecraft.carrot_on_a_stick
scoreboard objectives add pressedleave minecraft.used:minecraft.warped_fungus_on_a_stick

scoreboard objectives add daytime dummy
scoreboard objectives add creepertick dummy
scoreboard objectives add withertick dummy
scoreboard objectives add gillair dummy
scoreboard objectives add mhealth health "Health"
scoreboard objectives add mmobhealth dummy
scoreboard objectives add mmaxhealth dummy
scoreboard objectives add mhealed dummy
scoreboard objectives add mfood food

scoreboard objectives add moperator dummy
scoreboard objectives add moperand dummy
scoreboard objectives add muserid dummy

scoreboard objectives add dragonsecond dummy
scoreboard objectives add dragoncountdown dummy
scoreboard objectives add dragontick dummy

scoreboard objectives add zpiglinangertmp dummy
scoreboard objectives add zpiglinanger dummy

scoreboard objectives add mctrdeathcounter deathCount
scoreboard objectives add mctrdeathwait health
scoreboard objectives add mctrdeathtick dummy
scoreboard objectives add mctrdeathcount deathCount

scoreboard objectives add mctrlmobs.options.hardcore trigger
scoreboard objectives add mctrlmobs.options.craft trigger


#players won't be healed anymore
scoreboard players reset * mhealed


team add ctrlmobs "Control Mobs"
team add ctrlpiglins "Control Piglins"

forceload add 0 0
execute in minecraft:the_nether run forceload add 0 0


bossbar add mctrlmobs:ender_dragon "Ender Dragon"
bossbar set mctrlmobs:ender_dragon color pink
bossbar set mctrlmobs:ender_dragon max 200

bossbar add mctrlmobs:wither "Wither"
bossbar set mctrlmobs:wither color purple
bossbar set mctrlmobs:wither max 300